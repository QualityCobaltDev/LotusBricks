      ...
      <article className="space-y-8">
        <header className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          ...
          <aside className="h-fit rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap gap-2"><Badge tone="draft">{listing.propertyType}</Badge>{listing.isVerified ? <Badge tone="verified">Verified</Badge> : null}{listing.isFeatured ? <Badge tone="featured">Featured</Badge> : null}</div>
            <h1 className="mt-3 text-2xl font-bold text-neutral-900">{listing.title}</h1>
            <p className="mt-1 text-neutral-600">{listing.addressLine}, {listing.district}, {listing.city}</p>
            <p className="mt-4 text-3xl font-bold text-primary-700">${listing.priceUsd.toLocaleString()} {listing.intent === "rent" ? <span className="text-lg">/ month</span> : null}</p>
            <p className="mt-2 text-sm text-neutral-600">Owner/agent: {listing.ownerName} ({listing.ownerRole})</p>
            <LeadForm listingSlug={listing.slug} listingTitle={listing.title} />
            <p className="mt-2 text-xs text-neutral-500">Immediate support: <a href={`tel:${siteConfig.contactPhoneHref}`}>{siteConfig.contactPhoneDisplay}</a> · <a href={`mailto:${siteConfig.contactEmail}`}>{siteConfig.contactEmail}</a></p>
          </aside>
        </header>

        <section className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-5">
            <div><h2 className="text-xl font-semibold text-neutral-900">About this property</h2><p className="mt-2 text-neutral-700">{listing.description}</p></div>
            <div><h2 className="text-xl font-semibold text-neutral-900">Property facts</h2><ul className="mt-2 grid grid-cols-2 gap-2 text-sm text-neutral-700 md:grid-cols-4"><li className="rounded-lg bg-white p-3">Beds: {listing.bedrooms ?? "N/A"}</li><li className="rounded-lg bg-white p-3">Baths: {listing.bathrooms ?? "N/A"}</li><li className="rounded-lg bg-white p-3">Floor: {listing.floorAreaSqm ?? "N/A"} sqm</li><li className="rounded-lg bg-white p-3">Land: {listing.landAreaSqm ?? "N/A"} sqm</li></ul></div>
