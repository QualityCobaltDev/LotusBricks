    ...
    <header className="sticky top-0 z-30 border-b border-neutral-200 bg-white/95 backdrop-blur">
      <nav className="mx-auto max-w-7xl px-4 py-3 md:px-6" aria-label="Primary">
        <div className="flex items-center justify-between gap-4">
          ...
          <ul className="hidden items-center gap-6 md:flex">
            {siteConfig.nav.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-sm text-neutral-700 hover:text-primary-700">
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <details className="mt-3 md:hidden">
          <summary className="cursor-pointer rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-medium text-neutral-800">
            Menu
          </summary>
          <div className="mt-3 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
            <ul className="space-y-3">
              {siteConfig.nav.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="block text-sm text-neutral-700 hover:text-primary-700">
                    {link.label}
                  </Link>
                </li>
              ))}
