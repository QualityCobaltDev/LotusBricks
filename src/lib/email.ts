...
function renderTemplate(data: EmailTemplateData, contact: { phoneDisplay: string; email: string }) {
  const rows = (data.rows ?? [])
    .map((r) => `<tr><td style="padding:6px 0;color:#667085;font-size:13px;width:170px">${r.label}</td><td style="padding:6px 0;color:#101828;font-size:14px">${r.value}</td></tr>`)
    .join("");

  return `<div style="background:#f4f6fb;padding:24px 12px;font-family:Arial,sans-serif"><table role="presentation" style="max-width:640px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e4e7ec"><tr><td style="background:#123b7a;color:#fff;padding:18px 24px;font-weight:700;font-size:20px">RightBricks</td></tr><tr><td style="padding:24px"><h1 style="margin:0 0 12px;font-size:22px;color:#101828">${data.heading}</h1><p style="margin:0 0 12px;color:#344054;line-height:1.5">${data.intro}</p>${rows ? `<table role="presentation" style="width:100%;border-top:1px solid #eaecf0;border-bottom:1px solid #eaecf0;padding:8px 0;margin:14px 0">${rows}</table>` : ""}${data.ctaLabel && data.ctaHref ? `<a href="${data.ctaHref}" style="display:inline-block;margin-top:10px;background:#175cd3;color:#fff;padding:10px 14px;border-radius:8px;text-decoration:none;font-weight:600">${data.ctaLabel}</a>` : ""}<p style="margin:16px 0 0;color:#344054">${data.closing ?? "Our team will contact you shortly via phone or email."}</p></td></tr><tr><td style="background:#f9fafb;color:#475467;padding:16px 24px;font-size:13px">Contact: ${contact.email} · ${contact.phoneDisplay}</td></tr></table></div>`;
}
