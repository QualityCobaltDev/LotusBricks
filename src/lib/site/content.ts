export const trustStats = [
  { label: "Verified listings", value: "12,400+" },
  { label: "Monthly qualified inquiries", value: "8,900" },
  { label: "Metro regions covered", value: "34" },
  { label: "Average response time", value: "< 2 hrs" }
];

export const testimonials = [
  {
    quote:
      "RightBricks helped us evaluate properties faster with complete documentation and transparent pricing history.",
    name: "Elena Park",
    role: "Investment Director, Northgate Capital"
  },
  {
    quote:
      "The listing quality is exceptional. We moved from discovery to signed lease in nine days with zero surprises.",
    name: "Marcus Greene",
    role: "Operations Lead, Orbit Mobility"
  },
  {
    quote:
      "As a relocation advisor, I trust RightBricks because every listing is media-rich, current, and professionally verified.",
    name: "Lina Darzi",
    role: "Senior Relocation Consultant"
  }
];

export const faqs = [
  {
    q: "How are listings verified?",
    a: "Our review team validates ownership signals, media authenticity, and key documentation before a listing receives the verified badge."
  },
  {
    q: "Can I compare neighborhoods and property types?",
    a: "Yes. You can filter by district, property profile, price range, and key specifications to shortlist opportunities quickly."
  },
  {
    q: "Is RightBricks suitable for investors and agencies?",
    a: "Absolutely. Growth and Scale plans include portfolio tools, priority support, and partner workflows for teams."
  }
];
